...
- in the 'escalate' case, it will call a plugin to escalate the request.  The default one is to send the request directly back to the user via the agent's MCP conversation.  But other, more secure options can be added (so, for example, Keep writes the approval URL to slack instead)
...
**PDPClient interface** — pluggable. Primary implementation calls the OPA (Open Policy Agent) REST API. The request body sent to the PDP is the full `EnrichedMCPRequest` (tool call + user identity + escalation tokens + session metadata). The PDP returns `allow`, `deny`, or `escalate` with a reason string and an audit ID.  In the case of `escalate` it will also return the specific argument permissions needed by the agent to perform the work. 

So, for example, If the user wants to view the contents of the file `/var/log/whatever.log`, the PDP will return a description of that grant. This is an example: `{ "type":"prefix", "path":"path", "data":"/var/log/whatever.log" }`
...
- **Url** : generate a signed escalation_request JWT which includes the grants provided by the PDP and attaches that JWT as the parameter to a URL (which resolves to portcullis-guard) 

**URL Handler**

Escalation is asynchronous and the actual escalation tokens are not generated by Portcullis-Keep. Portcullis-keep returns a response indicating there was a tool error, and includes the URL and instructions to the agent to show the URL to the user. Portcullis-gate surfaces this to the agent (MCP tool error). 

When the user clicks on the link, it leads to the portcullis-guard web interface.  This shows the details of the JWT, which indicates the type of functionality that the AI is requesting.  It is up to the user to approve the request. If the user approves the request, portcullis-guard creates the `escalation_token` and presents it to the user, so the user can add it to the portcullis-gate token store.

The enterprise workflow resolves asynchronously. The escalation process is stateless on the portcullis-keep side - it passes the request to the workflow handler, and then forgets. This potentially creates a risk of multiple requests for the same user for the same resource, but that problem can be resolved by the workflow.

**Decision Logger** — optionally batches and forwards fast-path decisions from gate instances to a configurable SIEM or audit system. Uses a channel-based architecture with configurable buffer sizes, flush intervals, and batch sizes. Logs are compressed with gzip before transmission. Supports console logging for development and remote HTTP endpoints for production. All decision logs include complete context: timestamp, session ID, request ID, user ID, tool name, decision, reason, and arguments.
