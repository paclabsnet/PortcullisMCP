...
func parseFlags(argv []string) (cliArgs, error) {
	fs := flag.NewFlagSet("portcullis-guard", flag.ContinueOnError)
	var a cliArgs
	fs.StringVar(&a.config, "config", "/etc/portcullis/guard.yaml", "path to guard config file")
...
func applyLogging(cfg guard.Config, args cliArgs) error {
