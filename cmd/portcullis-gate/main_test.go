...
func TestParseFlags(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			argv:         []string{},
			wantLogLevel: "",
			wantConfig:   "~/.portcullis/gate.yaml",
		},
		{
			name:         "log-level flag is parsed",
			argv:         []string{"--log-level", "debug"},
			wantLogLevel: "debug",
			wantConfig:   "~/.portcullis/gate.yaml",
		},
		{
			name:         "config flag is parsed",
			argv:         []string{"--config", "/etc/gate.yaml"},
			wantLogLevel: "",
			wantConfig:   "/etc/gate.yaml",
		},
		{
			name:         "both flags parsed together",
			argv:         []string{"--log-level", "warn", "--config", "/etc/gate.yaml"},
			wantLogLevel: "warn",
			wantConfig:   "/etc/gate.yaml",
		},
		{
...
func TestApplyLoggingForwardsArgs(t *testing.T) {
	...
	if err := applyLogging(gate.Config{Mode: "dev"}, cliArgs{logLevel: "debug"}); err != nil {
		t.Fatalf("applyLogging: %v", err)
	}
	if capturedLevel != "debug" {
		t.Errorf("capturedLevel = %q, want %q", capturedLevel, "debug")
	}
	if capturedMode != "dev" {
